...
void usage(void) {
	...
	printf("\t-h\t\tPrint human-readble names for services.\n");
...
int v_print_hostinfo(struct in_addr addr, const struct nb_host_info* hostinfo, char* sf, int hr) {
	int i, unique;
	uint8_t service; /* 16th byte of NetBIOS name */
	char name[16];
	char* sname;
...
int main(int argc, char *argv[]) {
	int timeout=1, verbose=0, use137=0, ch, dump=0, bandwidth=0, send_ok=0, hr=0;
        extern char *optarg;
        extern int optind;
	char* target_string;
	char* sf=NULL;
	...
	while ((ch = getopt(argc, argv, "vrdqhm:s:t:b:")) != -1)
		switch (ch) {
             	case 'v':
                	verbose = 1;
               		break;
	...
	if(argc!=1) usage();
	if((target_string=malloc(strlen(argv[0])+1))==NULL) 
	...
	if(!(quiet || sf)) printf("Doing NBT name scan for adresses from %s\n\n", target_string);
    	free(target_string);
	...
	if( timercmp(&select_timeout, &send_interval, > ) ) {
		select_timeout.tv_sec = send_interval.tv_sec;
		select_timeout.tv_usec = send_interval.tv_usec;
	};

	if(!(quiet || verbose || dump || sf)) print_header();
		...
		gettimeofday(&transmit_started, NULL);
			...
			if(FD_ISSET(sock, fdsr)) {
                		if ( (size = recvfrom(sock, buff, BUFFSIZE, 0,
                        	...
                        	err_print("Recvfrom failed", quiet);
                        	continue;
                		};
				...
				if(insert(scanned, ntohl(dest_sockaddr.sin_addr.s_addr))) {
					/* printf("%s: timestamp: %d\n", inet_ntoa(dest_sockaddr.sin_addr), hostinfo->header->transaction_id); */
